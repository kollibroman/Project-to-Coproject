...
%%--biblatex--%%]{biblatex}
%%--biblatex--%%\addbibresource{dokumentacja.bib}
...
\title{Projekt aplikacji ułatwiającej współpracę przy grupowych projektach studenckich oraz pozyskiwanie potencjalnych inwestorów} % INFO: tytuł pracy w języku polskim 
...
\titleEN{A project for an app designed to facilitate collaboration on student group projects and attract potential investors } % INFO: tytuł pracy w języku angielskim
\author{Filip Kwiek}  % INFO: imię i nazwisko autora
...
\specjalnosc{Grafika i systemy multimedialne (IGM)} % INFO: nazwa specjalności
...
\date{WROCŁAW, 2027} % INFO: miejscowość, rok złożenia pracy dyplomowej
...
\bibliography{dokumentacja} %%--bibtex--%%
