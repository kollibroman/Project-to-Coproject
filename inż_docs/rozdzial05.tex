...
Cytowania powinny być zamieszczane w tekście z użyciem komendy \verb+\cite{}+. Jej argumentem powinien być klucz cytowanej pozycji (lub lista kluczy  rozdzielonych przecinkiem bez spacji, jeśli takich pozycji w danym miejscu cytuje się więcej) jaki jest używany w bazie danych bibliograficznych (plik \texttt{dokumentacja.bib}). 
Przykładem cytowania jest: ,,dobrze to opisano w~pracach~\cite{JS07,SQL2}'' (gdzie zastosowano komendę \verb?\cite{JS07,SQL2}? oraz na raz wypisano dwa klucze, rozdzielone przecinkiem bez spacji).

Podczas kompilacji dokumentów zawierających cytowania \LaTeX tworzy dodatkowy plik wynikowy, który jest konsumowany podczas uruchomienia wybranego narzędzie (odpowiedzialnego za sformatowanie referencji oraz wykazu literatury). Narzędzie to dostarcza  kolejnych pośrednich wyników kompilacji, które finalnie pozwalają \LaTeX{}-owi sformatować dokument.

Standardowy strumień pracy składa się więc z kilku kroków:
\begin{enumerate}
\item uruchomienie kompilacji: \texttt{pdflatex Dyplom.tex} (\LaTeX przeanalizuje dokument, zapisze informacje o cytowaniach w pliku pomocniczym, stworzy szkielet układu stron). 
\item wygenerowanie bazy bibliograficznej: \texttt{[narzędzie] Dyplom.tex} (narzędzie odczyta dane z pliku pomocniczego, posortuje rekordy, sformatuje je zgodnie ze wskazanym stylem, wynik zapisze w plikach pomocniczych).
\item uruchomienie kompilacji: \texttt{pdflatex Dyplom.tex} (\LaTeX wstawi sformatowaną bibliografię do dokumentu i przygotowuje referencje w tekście).
\item uruchomienie kompilacji: \texttt{pdflatex Dyplom.tex} (może być konieczne, by ostatecznie zaktualizowała się numeracja stron w spisie treści i odnośniki).
\end{enumerate}


Polecanymi narzędziami do tworzenia referencji i wykazów literatury są: \texttt{bibtex} oraz \texttt{biblatex}. Działają one na podobnej zasadzie (przetwarzają rekordy bibliograficzne zadeklarowane w pliku \texttt{bib}, korzystają z poleceń \verb+\cite+ umieszczonych w plikach \texttt{tex} i przygotowują formatowanie zgodnie z zadanym stylem). Jednak dość znacznie różnią się w szczegółach. Narzędzie \texttt{biblatex} rozpoznaje więcej różnych typów rekordów bibliograficznych niż \texttt{bibtex} oraz lepiej sobie radzi z kodowaniem UTF. Inaczej jest parametryzowane (domyślnie działa z~wykorzystaniem silnika \texttt{biber}) oraz korzysta z innych styli niż \texttt{bibtex}.
 
Niniejszy szablon przygotowano do pracy z \texttt{bibtex} oraz \texttt{biblatex}, przy czym aby użyć to drugie narzędzie trzeba odkomentować w pliku \texttt{Dyplom.tex} linijki zamarkowane komentarzem \verb+%%--biblatex--%%+ 
a następnie zakomentować linijki kończące się komentarzem \verb+%%--bibtex--%%+.
Ponadto należy nieco inaczej przygotować rekordy danych (z uwagi na inny zakres dostępnych typów rekordów oraz sposób działania styli).
...
Niekiedy tytuły prac zawierają wyrazy z dużymi i małymi literami. Takie tytuły należy brać w~podwójne nawiasy klamrowe, aby \texttt{bibtex} nie zamienił ich na postać, w której poza pierwszą literą pozostałe są małe. Jeśli jakiś cytowany zasób pochodzi z Internetu, to w metadanych odpowiadającego mu rekordu w pliku \texttt{bib} powinna pojawić się data dostępu. Poniżej pokazano przykład takiego rekordu. Proszę zwrócić uwagę, że adres url oraz datę dostępu umieszczono w~polu \texttt{note}. Jest to specyficzne dla zastosowanego stylu formatowania bibliografii narzędziem \texttt{bibtex}. Przy zastosowaniu narzędzia \texttt{biblatex} z silnikiem \texttt{biber} link i datę dostępu można umieścić w osobnych polach. Datę jednak trzeba podawać w notacji z myślnikami. Po kompilacji wygenerowane zostaną poprawne pozycję w wykazie literatury (silnik dopisze ,,dostęp'').
\begin{lstlisting}[caption=Przykład rekordu dla \texttt{bibtex}]
@INPROCEEDINGS{SQL2, 
  ...
  day = {15--19},
  year={2007}, 
note={\url{http://www.osti.gov/scitech/servlets/purl/922267} [dostęp 20 czerwca 2026]}
}
\end{lstlisting}


\begin{lstlisting}[caption=Przykład rekordu dla \texttt{biblatex} z silnikiem \texttt{biber}]
@INPROCEEDINGS{SQL2, 
  title={{A MySQL-based data archiver: preliminary results}}, 
  author={Bickley, M. and Slominski, Ch.},
  booktitle = {{Proceedings of ICALEPCS07}},
  ...
  day = {15--19},
  year={2007}, 
  url={http://www.osti.gov/scitech/servlets/purl/922267},
  urldate = {2026-06-20}
}
\end{lstlisting}

Oto inny przykład rekordu danych bibliograficznych:
\begin{lstlisting}[caption=Przykład rekordu typu \texttt{TechReport}]
@TechReport{JS07,
...
Dobrą praktyką jest zweryfikowanie, czy po skompilowaniu \LaTeX{}owego projektu wygląd wygenerowanego wykazu literatury spełnia oczekiwania. Często okazuje się, że nie wszystkie pola metadanych zostały poprawnie wyrenderowane (bo tak zadziałał wybrany styl) lub niespodziewanie zmieniona została wielkość liter bądź dopisane zostały jakieś dodatkowe słówka. 
